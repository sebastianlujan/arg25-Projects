...
- **Relayer (zkFisher)**: Off-chain service that monitors Arbitrum blocks, generates Merkle Trees, and creates snapshots for private state management
...
- **Aztec Protocol** - Privacy-focused blockchain for storing Merkle Tree snapshots and maintaining private state
- **Groth16** - Zero-knowledge proof system for generating proofs compatible with Arbitrum verification
...
- Implement relayer (zkFisher) architecture for block monitoring and private state snapshots
...
We created **Stylus (Rust) interfaces** for optimized interaction with EVVM Solidity-FHE contracts, enabling high-performance calls from Rust contracts. We developed the **EVVM-CAFHE** example that demonstrates the complete interaction between Stylus and EVVM with FHE, showing the encrypted data flow between both layers. We documented the hybrid architecture, workflows, and implementation guides in the [docs/](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs) folder.

## Relayer Architecture (zkFisher)

The relayer component (zkFisher) is a critical off-chain service that bridges Arbitrum and Aztec, enabling private state management for EVVM transactions.

### Architecture Overview

The relayer follows a multi-step process to capture, process, and store EVVM transaction data:

1. **Block Monitoring**: The relayer continuously monitors blocks generated by Arbitrum, filtering only blocks that contain at least one EVVM transaction.

2. **Merkle Tree Generation**: For each relevant block, the relayer extracts the block hash and uses it to build a Merkle Tree. This tree aggregates multiple block hashes, creating a cryptographic structure that enables efficient verification of block inclusion.

3. **Snapshot Creation**: Periodically (every N blocks, e.g., 20 blocks as an example value), the relayer creates a snapshot by:
   - Computing the Merkle Root from the accumulated block hashes
   - Sending this root hash to an Aztec contract for storage
   - Maintaining the private state of EVVM epochs in a privacy-preserving manner

4. **Zero-Knowledge Proof Generation** (Future Scope): The relayer will generate Groth16 proofs that validate the correctness of the Merkle Tree construction and block inclusion. These proofs will be verified on-chain in Arbitrum, enabling trustless verification of the relayer's work without exposing the underlying data.

### Key Components

- **Block Listener**: Monitors Arbitrum blocks and filters EVVM transactions
- **Merkle Tree Builder**: Constructs Merkle Trees from block hashes
- **Snapshot Manager**: Creates periodic snapshots and interacts with Aztec contracts
- **Proof Generator** (Planned): Generates Groth16 proofs for on-chain verification

### Integration Points

- **Arbitrum**: Source of blocks and EVVM transactions
- **Aztec**: Storage for Merkle Tree snapshots and private state
- **Arbitrum Verifier** (Planned): On-chain contract that verifies Groth16 proofs generated by the relayer

This architecture ensures that EVVM transaction history can be maintained privately while still enabling cryptographic verification of state transitions.

## Final Wrap-Up

_After Week 3, summarize your final state: deliverables, repo links, and outcomes._

- **Main Repository Link:** [GitHub Repository](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm)
...
- **Slides / Presentation (if any):**
  - **Litepaper:** [LITEPAPER.pdf](https://github.com/sebastianlujan/arg25-Projects/blob/invisible-zkevvm/invisible-zkevvm/stylus-contracts/litepaper/LITEPAPER.pdf)
  - Technical documentation available in [docs/](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs)
  - Implementation guides: [FHE_SETUP.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/FHE_SETUP.md), [TREASURY_GUIDE.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/TREASURY_GUIDE.md), [STAKING_ENCRYPTED_INPUTS.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/STAKING_ENCRYPTED_INPUTS.md)
  - Implementation plan: [IMPLEMENTATION_PLAN.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/IMPLEMENTATION_PLAN.md)
...
- ✅ Relayer (zkFisher) architecture design and implementation for block monitoring and Merkle Tree generation
...
- **Block monitoring**: We implemented off-chain services that monitor blockchain events and filter relevant transactions
- **Merkle Tree construction**: We learned to build Merkle Trees from block hashes for efficient cryptographic verification
- **Private state snapshots**: We explored how to maintain private state in Aztec while enabling verification through Merkle proofs
- **Zero-knowledge proofs**: We researched Groth16 proof systems for generating proofs compatible with Arbitrum's verification infrastructure
...
- Complete relayer implementation with Aztec contract integration for snapshot storage
- Implement Groth16 proof generation in the relayer for on-chain verification
...
For more technical details about the project, see the documentation in the [docs/](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs) folder:

- **[FHE_SETUP.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/FHE_SETUP.md)** - Complete Zama FHE setup guide
- **[TREASURY_GUIDE.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/TREASURY_GUIDE.md)** - TreasuryVault usage guide with FHE
- **[STAKING_ENCRYPTED_INPUTS.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/STAKING_ENCRYPTED_INPUTS.md)** - Encrypted inputs guide for Staking
- **[IMPLEMENTATION_PLAN.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/IMPLEMENTATION_PLAN.md)** - Detailed implementation plan
- **[PROGRESS.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/PROGRESS.md)** - Detailed progress report
- **[SPEC.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/docs/SPEC.md)** - Technical specifications of the project
- **[DEPLOYMENT_SUMMARY.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/DEPLOYMENT_SUMMARY.md)** - Deployment summary
- **[QUICK_DEPLOY.md](https://github.com/sebastianlujan/arg25-Projects/tree/invisible-zkevvm/invisible-zkevvm/QUICK_DEPLOY.md)** - Quick deployment guide
